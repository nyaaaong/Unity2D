...
public class ItemManager : Global
{
	...
	[SerializeField]
	private float m_LootRate = 25.0f;
	...
	public static void CreateItem(Vector3 pos)
	{
		...
		if (!CharacterManager.PlayerHasWeaponAll() || !IsDroppedWeapAll())
		{
			bool Loop = false;
			...
			do
			{
				...
				{
					case Item_Type.Rifle:
						if (!CharacterManager.PlayerHasWeapon(type) && !IsDroppedWeap(Item_Type.Rifle))
						{
							Loop = false;
						...
						break;
					case Item_Type.Sniper:
						if (!CharacterManager.PlayerHasWeapon(type) && !IsDroppedWeap(Item_Type.Sniper))
						{
							Loop = false;
